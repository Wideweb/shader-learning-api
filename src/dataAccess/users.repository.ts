...
export class UserRepository {
  public async findUserById(userId: number): Promise<UserModel> {
    const result = await dbConnection.query<UserModel>(`SELECT * FROM Users WHERE Id = ${userId} LIMIT 1`);
    return result[0];
  }

  public async findUserByEmail(email: string): Promise<UserModel> {
    const result = await dbConnection.query<UserModel>(`SELECT * FROM Users WHERE Email = '${email}' LIMIT 1`);
    return result[0];
  }

  public async findUserByName(userName: string): Promise<UserModel> {
    const result = await dbConnection.query<UserModel>(`SELECT * FROM Users WHERE UserName = '${userName}' LIMIT 1`);
    return result[0];
  }
